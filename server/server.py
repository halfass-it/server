...
class Server:
  ...
  async def handle_client(self, reader, writer):
    ...
    self.logger.info(
      f'[OPENED] Connected to client from {client_ip}:{client_port} at {self.ip}:{self.port}'
    )
    try:
      while True:
        ...
        try:
          ...
          self.logger.info(
            f"[UPSTREAM]: '{packet.data}' from {client_ip}:{client_port}"
          )
        except Exception as e:
          self.logger.error(
            f'[ERROR] Error handling client data, closing connection: {e}'
          )
          break
        try:
          ...
          self.logger.info(
            '[CLOSED] Error handling client data, closing connection: {e}'
          )
          break
        ...
        self.logger.info(
          '[DOWNSTREAM]: '
          + str({
            'auth': command_packet.auth.data,
            'game': command_packet.game.data,
          })
      ...
      self.logger.error(
        f'[ERROR] Error handling client data, closing connection\n[STACKTRACE]: {e}'
      )
    finally:
      try:
        writer.close()
        ...
        self.logger.info(
          f'[CLOSED] Disconnected from {client_ip}:{client_port}'
        )
      except Exception as e:
        self.logger.error(
          f'[ERROR] Error closing connection\n[STACKTRACE]: {e}'
        )
